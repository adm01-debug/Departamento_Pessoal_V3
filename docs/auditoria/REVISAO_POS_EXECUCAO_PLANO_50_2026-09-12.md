...
| Vitest                   | 468 arquivos; 4.927 aprovados; 1 ignorado; cobertura real: 61,94% statements, 57,12% branches, 54,42% functions e 66,25% lines                                                 |
| Edge Functions           | 60/60 entrypoints aprovados no `deno check`; 110 testes Deno aprovados                                                                                                         |
| Banco descartável        | 26/26 scripts PostgreSQL aprovados, incluindo reaplicação, concorrência, RLS, negações exatas e preflight fail-closed                                                          |
...
| RV-16 gates falsos-verdes   | Edge agora bloqueante, agenda incluída, 26 SQL suites no CI, formatação real, contrato bridge/frontend e URL canônica validados; DB/E2E autenticado não rodam código de PR | checks remotos verdes e proteção de branch revisada                                 |

### Ordem de promoção, sem atalhos

1. Publicar o lote em branch e PR, mantendo `AUDIT_REPORT.pdf` fora do commit.
2. Executar o workflow canônico em modo `validate`, que aplica exatamente 19 migrations numa única transação revertida.
3. Executar o mesmo lote em modo `apply`; o CLI registra somente essas 19 versões no ledger — não há `db push` nem `migration repair` em massa.
