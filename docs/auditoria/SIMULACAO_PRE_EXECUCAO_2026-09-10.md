...
| S08 | Replay integral de migrações, E50-009–019/030–039 | `supabase start` criou stack efêmera e reproduziu o replay histórico: 528 migrations aplicaram; a 529ª (`20260724100000_p3_054_mv_telemetry_dashboard.sql`) falhou com `SQLSTATE 42703`, pois referencia `query_telemetry.bytes_sent`, coluna inexistente. A stack foi automaticamente parada.                        | **Vermelho**    | Não editar a história para “seguir”. Tratar a migration como obsoleta, usar baseline/squash em staging hospedado e comparar inventário físico antes de qualquer mudança no canônico. |
...
- **E50-009–019 diagnóstico confirmado:** o replay completo em stack local atingiu o mesmo ponto de interrupção documentado no drift (ordinal 529, `bytes_sent` inexistente). Isso é evidência para a estratégia de baseline, não autorização para alterar a migration histórica ou reconciliar o ledger.
