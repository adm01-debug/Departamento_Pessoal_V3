...
try {
  ...
  const result = await page.evaluate(() => {
    ...
    all.forEach((el) => {
      ...
      const overLeftBy = containerLeft - r.left;
      const overRightBy = r.right - containerRight;
      if (overLeftBy > 0.5 || overRightBy > 0.5 || r.width > 700) {
        list.push({
          tag: el.tagName,
          ...
          overLeftBy: +overLeftBy.toFixed(1),
          overRightBy: +overRightBy.toFixed(1),
        });
      }
    });
    list.sort((a, b) => (b.overLeftBy + b.overRightBy) - (a.overLeftBy + a.overRightBy));
    return { containerLeft: +containerLeft.toFixed(1), containerRight: +containerRight.toFixed(1), matches: list.slice(0, 15) };
  });
