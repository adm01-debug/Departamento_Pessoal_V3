...
const RELATORIOS = [
  {
    ...
    description: 'Resumo mensal da folha',
    icon: BarChart2,
  ...
  {
    id: 'r2',
    title: 'Headcount por Depto',
    description: 'Colaboradores por departamento',
    icon: BarChart2,
...
describe('RelatoriosExportTab', () => {
  ...
  it('only exposes e-mail delivery for reports implemented by the Edge Function', () => {
    render(
      <RelatoriosExportTab
        relatorios={RELATORIOS}
        exportFormat="csv"
    ...
    expect(screen.getByRole('button', { name: 'Enviar Folha de Pagamento por e-mail' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Enviar Headcount por Depto por e-mail' })).not.toBeInTheDocument();
  });
