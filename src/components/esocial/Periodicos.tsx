      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Card className="border-border/30 shadow-xs bg-muted/5">
          <CardContent className="p-3 flex items-center gap-3">
            ...
            <div>
              <Label className="text-[9px] uppercase text-muted-foreground font-medium">Período Apuração</Label>
              <p className="text-xs font-medium">{dados.perApur}</p>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <div key={idx} className="space-y-2 border rounded-xl overflow-hidden shadow-xs">
          <div className="bg-muted/50 px-4 py-2 border-b flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Demonstrativo: {dm.ideDmDev}</span>
            </div>
            ...
            <Table>
              <TableHeader className="bg-muted/20">
                <TableRow className="hover:bg-transparent">
                  <TableHead className="h-7 text-[9px] font-medium uppercase tracking-wider">Cód. Rubrica</TableHead>
                  <TableHead className="h-7 text-[9px] font-medium uppercase tracking-wider text-right">Valor Informado</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {dm.infoPerApur?.ideEstabLot?.[0]?.detVerbas?.map((v: any, vIdx: number) => (
                  <TableRow key={vIdx} className="hover:bg-muted/10 border-border/10">
                    <TableCell className="py-2 text-[11px] font-mono font-medium text-primary">{v.codRubr}</TableCell>
                    <TableCell className="py-2 text-[11px] text-right font-display font-medium">
  ...
  return (
    <div className="space-y-4 font-body">
      <div className="flex items-center gap-2 mb-2 p-2 bg-primary/5 rounded-lg border border-primary/10">
        ...
        <span className="text-xs font-medium uppercase tracking-wider text-primary">Rendimentos Pagos ao Trabalhador</span>
      </div>
          ...
          <Card key={idx} className="border-border/30 shadow-xs hover:shadow-md transition-all">
            <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium font-display">Data de Pagamento: {p.dtPgto}</p>
              ...
              <div className="flex flex-col items-end border-t sm:border-t-0 pt-2 sm:pt-0 border-dashed border-border/50">
                <Label className="text-[9px] uppercase text-muted-foreground font-medium">Competência Tributária</Label>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <FileText className="h-3.5 w-3.5 text-primary" />
                  <span className="text-sm font-display font-medium text-primary">{dados.perApur}</span>
                </div>
              </div>
            </CardContent>
          </Card>
