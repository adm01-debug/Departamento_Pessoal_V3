...
export type WfNodeType = 'start' | 'end' | 'aprovador' | 'gateway_xor' | 'gateway_and' | 'email' | 'delay' | 'webhook';
...
const NODE_TEMPLATES: NodeTemplate[] = [
  ...
  },
  {
    ...
    icon: GitMerge,
    color: 'bg-info',
    description: 'Etapa de aprovação humana',
    hasConfig: true,
  },
  {
    type: 'gateway_xor',
    ...
    icon: GitMerge,
    color: 'bg-warning',
    description: 'Decisão exclusiva (sim/não)',
    hasConfig: true,
  },
  {
    type: 'gateway_and',
  ...
  },
  {
    ...
    icon: Mail,
    color: 'bg-chart-1',
    description: 'Envia e-mail automático',
    hasConfig: true,
  },
  {
    type: 'delay',
...
// ── Path helpers ────────────────────────────────────────────────
function bezierPath(x1: number, y1: number, x2: number, y2: number, dx = Math.abs(x2 - x1) * 0.5): string {
...
export function WorkflowDesigner({ initialNodes = [], initialConnections = [], onSave, disabled }: WfDesignerProps) {
  ...
  const addNode = (type: WfNodeType, x: number, y: number) => {
    if (nodes.length >= 50) {
      toast.error('Limite de 50 nós atingido');
      return;
    }
    ...
    const newNode: WfNode = {
      ...
      config:
        type === 'aprovador'
          ? { nivel: 1, sla_horas: 48, papel: 'gestor' }
          : type === 'email'
            ? { template: 'default', destinatario: '{{colaborador.email}}' }
            : type === 'delay'
              ? { duracao_horas: 24, tipo: 'fixo' }
              : type === 'webhook'
                ? { url: '', metodo: 'POST', body: '{}' }
                : {},
    };
  ...
  const validate = useCallback(() => {
    ...
    for (const n of nodes) {
      if (hasCycle(n.id)) {
        errors.add(n.id);
        toast.error('Ciclo detectado no fluxo!');
        break;
      }
    }
    ...
    <div className="flex gap-4 h-[600px]">
      ...
      <Card className="w-56 flex-shrink-0 rounded-2xl border-border/30 overflow-hidden">
        ...
        <CardContent className="p-2 space-y-1 overflow-y-auto">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="w-full h-7 rounded-lg text-xs text-destructive"
            onClick={() => {
              setNodes([]);
              setConnections([]);
      ...
      <div className="flex-1 relative rounded-2xl border border-border/30 overflow-hidden bg-muted/10">
            ...
            <g transform={`scale(${zoom}) translate(${pan.x / zoom}, ${pan.y / zoom})`}>
              ...
              {pendingFrom &&
                ...
                (() => {
                  ...
                  return (
                    <path d={path} stroke="#3b82f6" strokeWidth="2" fill="none" strokeDasharray="6 3" opacity="0.7" />
                  );
                })()}
              ...
              {nodes.map((node) => {
                ...
                if (node.type === 'gateway_xor' || node.type === 'gateway_and') {
                  return (
                    ...
                    >
                      ...
                      {/* Diamond shape */}
                      <polygon
                        points={`${GW_W / 2},0 ${GW_W},${GW_H / 2} ${GW_W / 2},${GW_H} 0,${GW_H / 2}`}
                        fill={template.color}
                        stroke={isSelected ? '#3b82f6' : hasError ? '#ef4444' : 'rgba(0,0,0,0.1)'}
                      ...
                      <circle
                        ...
                        onPointerDown={(e) => {
                          e.stopPropagation();
                          if (pendingFrom) completeConnection(node.id);
                  ...
                  return (
                    ...
                    >
                      ...
                      <Play
                        ...
                        style={{ position: 'absolute', left: node.x + NODE_W / 2 - 10, top: node.y + NODE_H / 2 - 10 }}
                      />
                      <circle
                        cx={NODE_W}
                        cy={NODE_H / 2}
                ...
                return (
                  ...
                  >
                    ...
                    {/* Config indicator */}
                    {template.hasConfig && <circle cx={NODE_W - 10} cy="10" r="4" fill="rgba(255,255,255,0.4)" />}
                    {/* Output port (right) */}
                    <circle
                      cx={NODE_W}
                      cy={NODE_H / 2}
        ...
        <div className="absolute bottom-4 right-4 flex gap-2">
          <Button
            size="sm"
            className="rounded-xl bg-gradient-to-r from-primary to-primary-glow font-body shadow-lg"
            onClick={handleSave}
          >
              ...
              <>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateNode(selectedNode.id, { config: { ...selectedNode.config, nivel: Number(e.target.value) } })
                    }
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateNode(selectedNode.id, { config: { ...selectedNode.config, papel: e.target.value } })
                    }
                  >
              ...
              <>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateNode(selectedNode.id, { config: { ...selectedNode.config, template: e.target.value } })
                    }
                  >
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateNode(selectedNode.id, { config: { ...selectedNode.config, destinatario: e.target.value } })
                    }
                  />
                </div>
              </>
              ...
              <>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateNode(selectedNode.id, { config: { ...selectedNode.config, tipo: e.target.value } })
                    }
                  >
              ...
              <>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateNode(selectedNode.id, { config: { ...selectedNode.config, url: e.target.value } })
                    }
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateNode(selectedNode.id, { config: { ...selectedNode.config, metodo: e.target.value } })
                    }
                  >
            ...
            {selectedNode.type === 'gateway_xor' && (
              <div className="bg-warning/10 rounded-lg p-2 border border-warning/20">
                <p className="text-[10px] font-body text-muted-foreground">
                  Coneecte as saídas do gateway a diferentes alvos. Cada conexão representa uma condição (ex: "Aprovado"
                  / "Rejeitado").
                </p>
              </div>
            )}
