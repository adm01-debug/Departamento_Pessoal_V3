...
export function IPBlockingTab() {
  ...
  const bloquear = useMutation({
    mutationFn: async () => {
      ...
      if (!isValidIP(ip))
        throw new Error('Endereço IP inválido. Use formato IPv4 (ex: 192.168.1.1) ou CIDR (ex: 10.0.0.0/24).');
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="rounded-2xl">
              <DialogHeader>
                <DialogTitle className="font-display">Bloquear IP</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Tentativas de acesso suspeitas"
                ...
                <Button
                  className="w-full rounded-xl bg-gradient-to-r from-destructive to-warning"
                  onClick={() => bloquear.mutate()}
                  disabled={!form.ip_address || bloquear.isPending}
                >
              ...
              <TableBody>
                {blockedIps.map((ip: any) => {
                  ...
                  return (
                    <TableRow
                      key={ip.id}
                      className={cn('hover:bg-accent/30 transition-colors', !ativo && 'opacity-50')}
                    >
                      ...
                      <TableCell>
                        <Badge
                          ...
                          className={cn(
                            'text-xs font-body',
                            ip.permanent ? 'border-destructive/30 text-destructive' : 'border-warning/30 text-warning'
                          )}
                        >
                      ...
                      <TableCell>
                        <Badge
                          className={cn(
                            'border-0 text-xs font-body',
                            ativo ? 'bg-destructive/15 text-destructive' : 'bg-muted text-muted-foreground'
                          )}
                        >
                      ...
                      <TableCell>
                        <Button
                          ...
                          onClick={() => desbloquear.mutate(ip.id)}
                        >
