...
describe('authService.resetPassword', () => {
  ...
  it('returns success when password update succeeds', async () => {
    ...
    const result = await authService.resetPassword('N3wSecret@123');
    expect(result).toMatchObject({ success: true });
    expect(mockUpdateUser).toHaveBeenCalledWith({ password: 'N3wSecret@123' });
  });

  it('throws when supabase auth returns error', async () => {
    mockUpdateUser.mockResolvedValue({ error: { message: 'Weak password' } });
    await expect(authService.resetPassword('Str0ng@Passw1')).rejects.toThrow('Weak password');
  });

  it('throws with fallback message when error has no message', async () => {
    mockUpdateUser.mockResolvedValue({ error: {} });
    await expect(authService.resetPassword('Str0ng@Passw1')).rejects.toThrow('Falha ao atualizar senha');
  });
});
