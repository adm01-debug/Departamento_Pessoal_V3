...
import { ShieldAlert, Lock, Unlock, Globe, AlertTriangle, CheckCircle, History, ShieldCheck } from 'lucide-react';
...
export default function SegurancaPage() {
  ...
  const { data: blockedIps, isLoading: loadBlocked } = useQuery({
    queryKey: ['blocked_ips'],
    queryFn: securityService.getBlockedIps,
  });
  const { data: loginAttempts, isLoading: loadLogins } = useQuery({
    queryKey: ['login_attempts'],
    queryFn: securityService.getLoginAttempts,
  });
  const { data: geoBlocked, isLoading: loadGeo } = useQuery({
    queryKey: ['geo_blocked'],
    queryFn: securityService.getGeoBlockedAttempts,
  });
  ...
  const resolveAlertMutation = useMutation({
    mutationFn: ({ id }: { id: string }) => securityService.resolveAlert(id),
    onSuccess: () => {
  ...
  return (
    <>
      <PageTitle
        ...
        description="Monitoramento de tentativas de acesso e proteção do sistema"
      />
      <PageLayout
          ...
          <TabsList className="bg-muted/50 p-1 border border-border/20 rounded-xl overflow-x-auto h-auto">
            ...
            <TabsTrigger value="alerts" className="rounded-lg py-2 gap-2">
              ...
              {alerts?.filter((a) => !a.resolved).length ? (
                <Badge variant="destructive" className="h-4 px-1 min-w-[16px] text-[10px]">
                  {alerts?.filter((a) => !a.resolved).length}
                </Badge>
              ) : null}
            </TabsTrigger>
            <TabsTrigger value="logins" className="rounded-lg py-2">
              Tentativas de Login
            </TabsTrigger>
            <TabsTrigger value="firewall" className="rounded-lg py-2">
              Firewall (IPs)
            </TabsTrigger>
            <TabsTrigger value="geo" className="rounded-lg py-2">
              Geo-Firewall
            </TabsTrigger>
            <TabsTrigger value="policies" className="rounded-lg py-2">
              Políticas RLS
            </TabsTrigger>
          </TabsList>
                  ...
                  <div className="space-y-4">
                    {loadAlerts ? (
                      ...
                      alerts?.slice(0, 5).map((alert) => (
                        <div
                          key={alert.id}
                          className="flex items-start justify-between p-3 rounded-xl border border-border/20 bg-muted/20 hover:bg-muted/30 transition-colors"
                        >
                          <div className="flex gap-3">
                            <div
                              className={`mt-1 p-1.5 rounded-full ${alert.severity === 'high' ? 'bg-destructive/10 text-destructive' : 'bg-warning/10 text-warning'}`}
                            >
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {alert.ip_address} • {format(new Date(alert.created_at), 'HH:mm', { locale: ptBR })}
                              </p>
                            </div>
                          </div>
                          {!alert.resolved && (
                ...
                <CardContent>
                  <div className="h-[300px] w-full flex items-center justify-center bg-muted/10 rounded-xl border border-dashed border-border/30">
                    <p className="text-sm text-muted-foreground italic text-center px-8">
                      Gráfico de tendências de tentativas de login por IP e horário disponível em breve
                    </p>
                  </div>
                </CardContent>
              </Card>
                      ...
                      <TableRow key={ip.id}>
                        ...
                        <TableCell className="text-xs">
                          {ip.permanent ? (
                            ...
                            format(new Date(ip.expires_at), 'dd/MM HH:mm')
                          ) : (
                            '—'
                          )}
                        </TableCell>
                        <TableCell className="text-right">
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-8 text-success hover:text-success hover:bg-success/10 rounded-lg gap-2"
                  ...
                  {loadGeo ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="text-center py-12 text-muted-foreground italic">
                        Nenhuma tentativa de acesso de países bloqueados
                      </TableCell>
                    </TableRow>
                  ) : (
                    geoBlocked?.map((geo) => (
                      <TableRow key={geo.id}>
                        ...
                        <TableCell className="text-[10px] text-muted-foreground truncate max-w-[300px]">
                          {geo.user_agent}
                        </TableCell>
                      </TableRow>
                    ))
