...
const relatorios: ReportDef[] = [
  {
    id: 'colaboradores',
    title: 'Resumo Colaboradores',
    description: 'Colaboradores ativos com dados completos',
    icon: Users,
  ...
  },
  {
    ...
    description: 'Relatório detalhado da folha',
    icon: DollarSign,
  ...
  },
  {
    id: 'ferias',
    title: 'Férias',
    description: 'Colaboradores com férias próximas ou vencidas',
    icon: Calendar,
  ...
  },
  {
    id: 'turnover',
    title: 'Turnover',
    description: 'Análise de rotatividade',
    icon: TrendingUp,
  ...
  },
  {
    ...
    description: 'Resumo INSS, FGTS, IRRF',
    icon: FileText,
...
const EMAIL_REPORT_TYPE = {
  colaboradores: 'lista_colaboradores',
  folha: 'folha_resumo',
  ferias: 'ferias_proximas',
} as const;
    ...
    case 'turnover': {
      ...
      const rows = Array.from(byMes.entries())
        ...
        .map(([competencia, v]) => ({
          ...
          taxa_turnover: ativos > 0 ? Number((((v.admissoes + v.desligamentos) / 2 / ativos) * 100).toFixed(2)) : 0,
        }));
...
export default function RelatoriosPage() {
  ...
  const { data: analytics } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: cols }, { data: deslig }, { data: admissoes }] = await Promise.all([
        supabase
          .from('colaboradores')
          .select('departamento, status, data_admissao, salario_base')
      ...
      (cols || [])
        .filter((c) => c.status === 'ativo' && c.salario_base)
        .forEach((c) => {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        months[`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`] = { admissoes: 0, desligamentos: 0 };
      }
      (admissoes || []).forEach((a) => {
        const m = a.data_prevista?.slice(0, 7);
        if (m && months[m]) months[m].admissoes++;
      });
      (deslig || []).forEach((d) => {
        const m = d.data_desligamento?.slice(0, 7);
        if (m && months[m]) months[m].desligamentos++;
      });
      const tendencia = Object.entries(months).map(([mes, v]) => ({ mes: mes.slice(5), ...v }));
      const custoTotal = (cols || [])
        .filter((c) => c.status === 'ativo')
        .reduce((acc, c) => acc + Number(c.salario_base || 0), 0);
      return {
        porDepartamento,
        salarioDistribuicao,
        tendencia,
        custoTotal,
        totalAtivos: (cols || []).filter((c) => c.status === 'ativo').length,
        totalDeslig: (deslig || []).length,
      };
    },
  });
  ...
  const handleSendEmail = async () => {
    ...
    const tipoRelatorio = EMAIL_REPORT_TYPE[emailDialog as keyof typeof EMAIL_REPORT_TYPE];
    if (!tipoRelatorio) {
      toast.error('Este relatório ainda não possui entrega por e-mail configurada.');
      return;
    }
    setSendingEmail(true);
    try {
      const resposta = await edgeFunctionsService.enviarRelatorioEmail({
        tipoRelatorio,
        formato: 'csv',
        emailDestinatario: emailTo.trim(),
        empresaId: empresaAtual.id,
        competencia: tipoRelatorio === 'folha_resumo' ? currentCompetenciaLocal() : undefined,
      });
      if (!resposta || typeof resposta !== 'object' || !('success' in resposta) || resposta.success !== true) {
        throw new Error('O provedor não confirmou o envio do relatório.');
      }
      toast.success('O provedor confirmou o envio do relatório.');
      setEmailDialog(null);
      setEmailTo('');
    } catch (e: unknown) {
      toast.error(safeErrorMessage(e, 'Erro ao gerar relatório.'));
        ...
        <Tabs defaultValue="analytics">
          <div className="flex justify-between items-center mb-6">
            <TabsList className="rounded-xl">
              <TabsTrigger value="analytics" className="rounded-lg font-body">
                📊 Analytics
              </TabsTrigger>
              <TabsTrigger value="exportar" className="rounded-lg font-body">
                📥 Exportar
              </TabsTrigger>
              <TabsTrigger value="agendados" className="rounded-lg font-body">
                ⏰ Agendamentos
              </TabsTrigger>
            </TabsList>
          ...
          <TabsContent value="analytics">
            <RelatoriosAnalyticsTab analytics={analytics} />
          </TabsContent>
          <TabsContent value="exportar">
            <RelatoriosExportTab
              ...
              onEmailOpen={setEmailDialog}
            />
          </TabsContent>
          <TabsContent value="agendados">
            <RelatoriosAgendadosTab empresaId={empresaAtual?.id || ''} />
          </TabsContent>
        </Tabs>
        <Dialog
          open={!!emailDialog}
          onOpenChange={(o) => {
            if (!o) setEmailDialog(null);
          }}
        >
          <DialogContent className="max-w-md rounded-2xl">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="font-body">Destinatário</Label>
                <Input
                  type="email"
              ...
              <p className="text-sm text-muted-foreground font-body">
                ...
                <span className="font-semibold text-foreground">
                  {relatorios.find((r) => r.id === emailDialog)?.title}
                </span>
              </p>
              <Button
                onClick={handleSendEmail}
                disabled={sendingEmail || !emailTo.trim()}
                className="w-full rounded-xl bg-gradient-to-r from-primary to-primary-glow font-body"
              >
