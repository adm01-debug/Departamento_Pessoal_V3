...
export default function FolhaPage() {
  ...
  const handleExportExcel = () => {
    ...
    exportarExcel('Histórico de Folhas', folhas, [
      'competencia',
      'tipo',
      'total_proventos',
      'total_descontos',
  ...
  const handleExportPDF = () => {
    if (!folhas?.length) return;
    exportarPDF('Histórico de Folhas', folhas, ['competencia', 'tipo', 'total_liquido', 'status']);
  };
  return (
    <>
      ...
      >
        ...
        <DataTableToolbar
          ...
          searchPlaceholder="Filtrar por competência (AAAA-MM)"
        />
        ...
        {isLoading ? (
          ...
          <>
            ...
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="hidden md:block rounded-2xl border border-border/30 overflow-hidden shadow-elevated"
            >
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {folhas.map((f) => (
                    <TableRow key={f.id} className="hover:bg-accent/30 transition-colors group">
                      ...
                      <TableCell className="text-right text-success font-body font-semibold tabular-nums">
                        {formatCurrency(f.total_proventos || 0)}
                      </TableCell>
                      <TableCell className="text-right text-destructive font-body font-semibold tabular-nums">
                        {formatCurrency(f.total_descontos || 0)}
                      </TableCell>
                      <TableCell className="text-right font-display font-bold tabular-nums">
                        {formatCurrency(f.total_liquido || 0)}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Dialog>
                            <DialogTrigger asChild>
                              <Button
                                variant="ghost"
                                size="icon"
                                aria-label="Auditoria e Conferência"
                                className="rounded-xl hover:bg-primary/10"
                                title="Auditoria e Conferência"
                              >
                          ...
                          <Button
                            ...
                            size="icon"
                            className="rounded-xl hover:bg-primary/10"
                            onClick={() => navigate('/folha/calcular')}
                    ...
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div>
                          ...
                          <p className="text-caption font-display font-bold text-success">
                            {formatCurrency(f.total_proventos || 0)}
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] text-muted-foreground font-body uppercase">Descontos</p>
                          <p className="text-caption font-display font-bold text-destructive">
                            {formatCurrency(f.total_descontos || 0)}
                          </p>
                        </div>
                        <div>
